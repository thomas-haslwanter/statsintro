...
If we add additional parameters to our model, we should somehow take that into consideration: because with additional parameters the residuals will \emph{always} become smaller, but the model may not become any better. Therefore for multiple regression the \emph{adjusted $R^2$} value (written as $\bar{R}^2$) is often used instead of $R^2$:
