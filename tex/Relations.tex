...
Pearson's correlation coefficient, sometimes also referred to as \emph{population correlation coefficient}, can take any value from -1 to +1. Examples are given in Figure \ref{fig:correlation}. Note that the formula for the correlation coefficient is symmetrical between $x$ and $y$.
...
\lstinputlisting[label=py:multivariate,caption=multivariate.py, language=Python]{../Code/multivariate.py}
\index{python}{multivariate}

\begin{figure}
  \centering
  \includegraphics[width=0.75\textwidth]{../Images/regression_wLegend.png}\\
  \caption{Regression, with confidence intervals for the mean, as well as for the predicted data.} \label{fig:regline}
\end{figure}
...
\lstinputlisting[label=py:fitLine,caption=fitLine.py, language=Python]{../Code/fitLine.py}
