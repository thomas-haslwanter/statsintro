...
The probability to find a value between $a$ and $b$ is given by the integral over the PDF in that range (see Fig. \ref{fig:PDF}), and the \gls{cdf_} \acrshort{cdf} specifies which percentage of the data below any given value (Figure \ref{fig:CDF}). Together, this gives us
...
\begin{itemize}
  \item In the first step, you create your distribution (e.g. \textsf{nd = stats.norm()}). Note that is a \emph{distribution} (in \emph{Python} parlance a "frozen distribution"\index{general}{frozen distribution}), not a function yet!
...
The \gls{sd_} \index{general}{standard deviation} is simply given by the square root of the variance:
...
\subsection{Normal Distribution} \label{sec:normalDistribution}\index{general}{distributions!normal}\index{general}{normal distribution}\index{general}{Gaussian distribution}
...
\end{equation}
where $ - \infty < x < \infty $, and $f_{\mu,\sigma}$ is the \gls{pdf_} (PDF) \index{general}{probability density function}.
...
Working with distribution functions in \emph{Python} takes a bit to get used to. But once you get the concept, it is marvellously easy. In my opinion, the most logical way is first to define the function, with all the parameters that it requires; and then, to use the methods of this function, e.g. $pdf$, or $cdf$:
